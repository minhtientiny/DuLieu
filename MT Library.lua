...
--[[	local NewDropShadow = Instance.new("ImageLabel")
	...
	VideoBackground.Video = "rbxassetid://"..MT_Video_Id
	if MT_Video_Display == "true" then
	...
	function MT_wind:Tab(tab_name)
		...
		function MT_tab:Button(text, func)
			...
			local MT_button = {}
			function MT_button:NewButton(new)
				ClickMe.Text = new.." 🖱️" or "Button 🖱️"
			end
			return MT_button
		end
		
		function MT_tab:Label(label_text, properties)
			...
			local MT_label = {}
			function MT_label:NewLabel(label_text, properties)
				TextHere.Text = label_text or "Text here"
				if properties then
					for i,v in pairs(properties) do
						pcall(function()
		...
		function MT_tab:Toggle(toggle_text, state, func)
			...
			local MT_toggle = {}
			function MT_toggle:NewText(new_t)
				TextHere.Text = "  "..new_t
			end
			function MT_toggle:NewState(new_state)
		...
		function MT_tab:Slider(txt, min, max, mid, func)
		...
		function MT_tab:Dropdown(txt,list,func)
			...
			local MT_drop = {}
			function MT_drop:NewText(ntxt)
				DropButton.Text = txt..": "..ntxt.." 💬"
			end
			function MT_drop:NewDrop(nlist)
		...
		function MT_tab:Textbox(txt,txt_hold,func)
	...
	mt:DraggingEnabled(SettingFrame)
